...
def home(id):
    try:
        if id is not None:
            ...
            return render_template("book_view.html", books=[book], g=g)
        else:
...
def add_book():
    try:
        ...
        if "error" in result:
            return render_template("books.html", error=result["error"], books=book_manager.getAllBooks(), g=g)
        
        return render_template("books.html", msg="Book added successfully!", books=book_manager.getAllBooks(), g=g)
    except Exception as e:
        return render_template('error.html', error=f"An error occurred: {str(e)}")
